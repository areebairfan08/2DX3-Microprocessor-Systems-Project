...
//Initialize input PM0 for push button
void PortM_Init(void){
	SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R11;          		//activate the clock for Port M
	while((SYSCTL_PRGPIO_R&SYSCTL_PRGPIO_R11) == 0){}; 		//allow time for clock to stabilize 
		GPIO_PORTM_DIR_R = 0b00000000;       						 		//make PM0 an input (0x0), PM0 is reading if the button is pressed or not 
		GPIO_PORTM_DEN_R = 0b00000001;											//enable digital I/O on PM0 
	return;
}

void PortH_Init(void){ //activate stepper motor
...
//  status = VL53L1X_SetInterMeasurementInMs(dev, 200); /* in ms, IM must be > = TB */
  ...
  while(1) {
		...
		if((GPIO_PORTM_DATA_R&0b00000001)==0) {
			...
			for(int i = 0; i < 16; i++) { //16 scans
				//wait until the ToF data is ready
				while (dataReady == 0){
          ...
          VL53L1_WaitMs(dev, 5);
				...
				//read the distance value from ToF sensor
				status = VL53L1X_GetDistance(dev, &Distance);					//The Measured Distance value

				status = VL53L1X_ClearInterrupt(dev); /* clear interrupt has to be called to enable next interrupt*/
		
				// print the resulted distance reading to UART
